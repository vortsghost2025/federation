...
echo
  ...
  # NOTE: --vps deep-probes are not yet implemented; container has no curl
  # binary and ssh+docker+python -c chains are quoting-fragile. This section
  # does only a docker ps check, which is fast and reliable.
  echo
  echo "  (--vps deep probes are not yet implemented; only docker ps is shown)"
