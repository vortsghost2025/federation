...
def think_about_world(r, contacts: dict | None = None, char_id: str = "") -> str:
    ...
    # ── Recent outcome effects (world-state consequences of your artifacts) ──
    try:
        effect_rec = r.hgetall(f"npc_outcome_effect:{cid}")
        if effect_rec and effect_rec.get("ts"):
            parts.append(
                f"↳ Your last artifact outcome: "
