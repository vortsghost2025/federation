...
def think_about_world(r) -> str:
    ...
    try:
        inbox = r.lrange(f"npc_messages:{CHAR_ID}:inbox", 0, 2)
    ...
    try:
        sent = r.lrange(f"npc_messages:{CHAR_ID}:sent", 0, 2)
    ...
    try:
        decs = r.lrange(f"npc_decisions:{CHAR_ID}", 0, 2)
        if decs:
            category_seq = []
            for item in decs:
                try:
            ...
            if category_seq:
                ...
                for line in reversed(category_seq):
                    parts.append(f"  - {line}")
    except Exception:
        pass
    ...
    # Partner mood only
    try:
        partner_id = "char_306" if CHAR_ID == "char_001" else "char_001"
...
def _consecutive_send_streak(r) -> int:
    ...
    Walks the npc_decisions list backwards from newest, stops at the
    first non-send_message decision. Returns 0 if none.
    """
    try:
        recent = r.lrange(f"npc_decisions:{CHAR_ID}", 0, 9)
        streak = 0
        for item in recent:
            try:
                d = json.loads(item)
...
{"category": "send_message", "reasoning": "...", "target": "char_306", "body": "message text", "description": "..."}
...
def execute_decision(decision: dict, r):
    ...
    if cat == "send_message":
        ...
        if target and target in CONTACTS and target != CHAR_ID:
            ...
            logger.info("[%s] Sent message to %s", CHAR_ID, target)
        ...
        partner_id = "char_306" if CHAR_ID == "char_001" else "char_001"
        try:
            partner_artifacts = r.lrange(f"npc_artifacts:{partner_id}", 0, 5)
            if partner_artifacts:
                summaries = []
                titles = []
                for a in partner_artifacts:
                    try:
                        obj = json.loads(a)
                        ...
                        pass
                result["action_taken"] = f"read {len(summaries)} artifacts from {partner_id}"
                ...
                _session_append(r, {
                    ...
                    "body": f"read {len(titles)} artifact(s)",
                })
            else:
