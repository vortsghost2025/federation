...
class GameState:
    ...
    def __init__(self):
        ...
        # Adapter state persistence (tech, quest, NPC, political subsystems)
        self.technology_data: Dict[str, Any] = {}
        self.quest_data: Dict[str, Any] = {}
        self.npc_data: Dict[str, Any] = {}
        self.political_data: Dict[str, Any] = {}
    ...
    def record_action(self, action_type: str, description: str, outcome: str,
        ...
        self.statistics.total_actions_made += 1
    ...
    def save_game(self, filepath: str) -> Dict[str, Any]:
        ...
        "political_data": self.political_data,
    }
    ...
    def load_game(self, filepath: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Restore federation core state (with fallbacks for missing keys)
            fed_data = data.get("federation", {})
            self.federation.morale = fed_data.get("morale", 0.5)
            self.federation.identity_strength = fed_data.get("identity_strength", 0.3)
            self.federation.stability = fed_data.get("stability", 0.6)
            self.federation.technological_level = fed_data.get("technological_level", 0.2)
            self.federation.military_power = fed_data.get("military_power", 0.3)
            self.federation.treasury = fed_data.get("treasury", 1000)
            self.federation.population = fed_data.get("population", 10000)
            self.federation.territory_size = fed_data.get("territory_size", 100.0)

            last_updated_str = fed_data.get("last_updated", None)
            if last_updated_str:
                try:
                    self.federation.last_updated = datetime.fromisoformat(last_updated_str)
                except (ValueError, TypeError):
                    self.federation.last_updated = datetime.now()

            # Restore subsystems (already uses .get() safely)
            self._restore_subsystems(data.get("subsystems", {}))

            # Restore statistics (with .get() fallbacks for keys added in later versions)
            stats_data = data.get("statistics", {})
            self.statistics.current_turn = stats_data.get("current_turn", 0)
            self.statistics.total_events_triggered = stats_data.get("total_events_triggered", 0)
            self.statistics.total_decisions_made = stats_data.get("total_decisions_made", 0)
            self.statistics.total_actions_made = stats_data.get("total_actions_made", 0)
            self.statistics.total_battles_fought = stats_data.get("total_battles_fought", 0)
            self.statistics.battles_won = stats_data.get("battles_won", 0)
            self.statistics.battles_lost = stats_data.get("battles_lost", 0)
            ...
            self.statistics.entities_conquered = stats_data.get("entities_conquered", 0)
            ...
            self.statistics.population_growth = stats_data.get("population_growth", 0)
            self.statistics.population_loss = stats_data.get("population_loss", 0)

            game_start_str = stats_data.get("game_start_time", None)
            if game_start_str:
                try:
                    self.statistics.game_start_time = datetime.fromisoformat(game_start_str)
                except (ValueError, TypeError):
                    self.statistics.game_start_time = datetime.now()

            self.statistics.playtime_hours = stats_data.get("playtime_hours", 0.0)

            # Restore action history
            self.action_history = self._deserialize_action_history(data.get("action_history", []))

            # Restore game phase and outcome
            phase_str = data.get("game_phase", "genesis")
            try:
                self.game_phase = GamePhase(phase_str)
            except ValueError:
                self.game_phase = GamePhase.GENESIS

            victory_str = data.get("victory_type", None)
            self.victory_type = VictoryType(victory_str) if victory_str else None
            self.defeat_reason = data.get("defeat_reason", None)
            self.is_game_over = data.get("is_game_over", False)

            # Restore timestamps
            creation_str = data.get("creation_timestamp", None)
            if creation_str:
                try:
                    self.creation_timestamp = datetime.fromisoformat(creation_str)
                except (ValueError, TypeError):
                    self.creation_timestamp = datetime.now()

            # Restore adapter subsystem data (HIGH risk — added later)
            self.technology_data = data.get("technology_data", {})
            self.quest_data = data.get("quest_data", {})
            self.npc_data = data.get("npc_data", {})
            self.political_data = data.get("political_data", {})

            # Restore metadata
            meta = data.get("metadata", {})
            self.save_count = meta.get("save_count", 0)
            self.last_save_timestamp = datetime.now()
    ...
    def _deserialize_action_history(self, data: List[Dict[str, Any]]) -> List[ActionRecord]:
        """Convert serialized action history back to ActionRecord objects."""
        records = []
        for entry in data:
            try:
                ...
                records.append(ActionRecord(
                    ...
                    morale_delta=entry.get("morale_delta", 0.0),
                    stability_delta=entry.get("stability_delta", 0.0),
                    treasury_delta=entry.get("treasury_delta", 0),
                ))
            except Exception:
                continue
        return records
