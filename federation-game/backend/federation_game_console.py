...
class ConsciousnessSheet:
    ...
    def complexity(self) -> float:
        trait_values = [self.morale, self.identity, self.anxiety, self.confidence, self.expansion_hunger, self.diplomacy_tendency]
        spread = max(trait_values) - min(trait_values) if trait_values else 0.0
        narrative_depth = min(1.0, (len(self.dreams) + len(self.prophecies) + len(self.archetypes) + len(self.traumas)) * 0.1)
...
class FederationConsole:
        ...
        def __init__(self, save_dir: str = "federation_saves"):
            ...
            self.state_manager: Optional[object] = None # GameState from federation_game_state
            self.faction_system: Optional[object] = None # FactionSystem from federation_game_factions
            self.npc_system: Optional[object] = None # NPCSystem from federation_game_npcs
            self.quest_system: Optional[object] = None # QuestSystem from federation_game_quests
            self.tech_tree: Optional[object] = None # TechTree from federation_game_technology
            self.history_arc: Optional[object] = None # HistoryArcOrchestrator from federation_game_history_arc
        ...
        def initialize_game(self):
            ...
            try:
                ...
                treasury = self.state_manager.federation.treasury if self.state_manager else 1000
            ...
            try:
                ...
                self.political_engine = PoliticalEngine(FACTION_IDS, self.state_manager.federation)
        ...
        def execute_turn(self) -> Dict[str, Any]:
            ...
            if self.quest_system and self.state_manager:
                try:
                    quest_results = self.quest_system.process_year(self.turn_number, self.state_manager)
                    if quest_results:
                        for quest_result in quest_results:
                            self._log_event('quest_completed', {
                                'quest_id': quest_result.get('quest_id', 'unknown'),
                                'quest_title': quest_result.get('quest_title', 'Unknown Quest'),
                            })
                    # Apply quest rewards to federation
                    if hasattr(self.quest_system, 'apply_rewards'):
                        self.quest_system.apply_rewards(quest_results, self.state_manager.federation)
            ...
            if self.npc_system and self.state_manager:
                modifiers = self.npc_system.process_year(self.turn_number, self.state_manager)
                if modifiers:
                    if 'morale_delta' in modifiers:
                        self.state_manager.federation.morale = max(0.0, min(1.0,
                            self.state_manager.federation.morale + modifiers['morale_delta']))
                    if 'identity_delta' in modifiers:
                        self.state_manager.federation.identity_strength = max(0.0, min(1.0,
                            self.state_manager.federation.identity_strength + modifiers['identity_delta']))
            # Political adapter: process yearly law generation
            if self.political_engine and self.state_manager:
                laws = self.political_engine.process_year(self.turn_number, self.state_manager.federation)
                if laws:
                    self.state_manager.political_data = self.political_engine.summary
                    for law in laws:
                        self._log_event('law_passed', {'law_id': law.get('law_id'), 'law_name': law.get('law_name'), 'year': self.turn_number, 'effects': law.get('effects')})
        ...
        def cmd_rivals(self):
            ...
            if self.rival_simulator:
                summary = self.rival_simulator.get_summary() if hasattr(self.rival_simulator, 'get_summary') else str(self.rival_simulator)
                print(f"\n{summary}\n")
            else:
        ...
        def cmd_history(self):
            ...
            print(f"  Events: {tl.get('event_count', 0)} | Factions: {tl.get('faction_count', 0)}")
            print(f"  Observers: {cs.get('observer_count', 0)} | Interpretations: {cs.get('total_interpretations', 0)}")
        ...
        def cmd_bridge(self):
            ...
            if self.chaosbringer:
                ...
                print(f"Warp Core: {wc.get('status', 'unknown')} (coherence: {wc.get('coherence', 0):.2f})")
        ...
        def cmd_politics(self):
            ...
            if recent:
                print("  Recent Laws:")
                for law in recent[-5:]:
                    print(f"   - {law.get('law_name', 'Unknown')} (ID: {law.get('law_id', '?')})")
            print()
            print(f"{'='*70}\n")
