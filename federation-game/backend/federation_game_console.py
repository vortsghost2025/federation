...
class ConsciousnessSheet:
    ...
    @property
    def awakeness(self) -> float:
        return (self.confidence + 1.0 - self.anxiety) / 2.0

    @property
    def memories_recorded(self) -> int:
        return len(self.dreams) + len(self.prophecies) + len(self.traumas)
    ...
    def complexity(self) -> float:
        trait_values = [
            ...
            self.anxiety,
            self.confidence,
            self.expansion_hunger,
            self.diplomacy_tendency,
        ]
        spread = max(trait_values) - min(trait_values) if trait_values else 0.0
        narrative_depth = min(
            1.0,
            (
                len(self.dreams)
        ...
        return (spread + narrative_depth) / 2.0
...
class EventCard:
    ...
    consequences: Dict[str, Dict[str, float]] = field(
        default_factory=dict
    )  # {choice_id: consciousness_impact}
    narrative_outcomes: Dict[str, str] = field(
        default_factory=dict
    )  # {choice_id: outcome_text}
    ...
    def resolve(self, choice: str) -> Tuple[str, Dict[str, float]]:
        ...
        outcome = self.narrative_outcomes.get(
            choice, "Your choice echoes through the federation..."
        )
    ...
    def get_random_event(self, event_type: Optional[EventType] = None) -> EventCard:
        ...
        if event_type:
            ...
            return (
                ...
                if filtered
                else random.choice(list(self.events.values()))
            )
...
class NarrativeGenerator:
    ...
    def generate_turn_narrative(
        turn_number: int, consciousness: ConsciousnessSheet
    ) -> str:
        """Generate narrative for turn begin"""
        mood = (
            "flourishing"
            if consciousness.health() > 0.7
            else "struggling"
            if consciousness.health() < 0.4
            else "steady"
        )
    ...
    def generate_rival_encounter(rival: RivalFederation) -> str:
        ...
        action = (
            ...
            if rival.expansion_tendency > 0.7
            else "consolidates position"
            if rival.expansion_tendency < 0.3
            else "expands cautiously"
        )
...
class TurnCycle:
    ...
    def execute_phase(self, phase: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if phase == "dream_generation":
            ...
            results["prophecy_insight"] = (
                "Symbols align in the collective unconscious..."
            )
...
class PersistenceManager:
    ...
    def __init__(self, save_dir: str = "federation_saves"):
        ...
        try:
            ...
            # Fallback to /tmp if save_dir is on read-only filesystem
            self.save_dir = Path("/tmp/federation_saves")
            try:
                self.save_dir.mkdir(exist_ok=True)
            except OSError:
    ...
    def save_game(
        self, game_data: Dict[str, Any], filename: Optional[str] = None
    ) -> Path:
...
class FederationConsole:
    ...
    def __init__(self, save_dir: str = "federation_saves"):
        ...
        # Integrated subsystems (wired from dormant modules)
        self.state_manager: Optional[object] = (
            None  # GameState from federation_game_state
        )
        self.faction_system: Optional[object] = (
            None  # FactionSystem from federation_game_factions
        )
        self.npc_system: Optional[object] = None  # NPCSystem from federation_game_npcs
        self.quest_system: Optional[object] = (
            None  # QuestSystem from federation_game_quests
        )
        self.tech_tree: Optional[object] = (
            None  # TechTree from federation_game_technology
        )
        self.history_arc: Optional[object] = (
            None  # HistoryArcOrchestrator from federation_game_history_arc
        )
        self.rival_simulator = None
        self.chaosbringer = None
        self.political_engine = None
        self.choice_ledger: List[Dict[str, Any]] = []  # Player decision tracking
        ...
        self.commands: Dict[str, Callable] = {
            ...
            "bridge": self.cmd_bridge,
            "politics": self.cmd_politics,
        }
    ...
    def initialize_game(self):
        ...
        try:
            ...
            self.state_manager = FederationGameState()
        except ImportError:
            logger.warning("federation_game_state not available")
        try:
            from federation_game_factions import build_faction_system

            self.faction_system = build_faction_system()
        except ImportError:
            logger.warning("federation_game_factions not available")
        try:
            from federation_game_npcs import NPCSystem

            self.npc_system = NPCSystem()
        except ImportError:
            logger.warning("federation_game_npcs not available")
        try:
            from federation_game_quests import QuestSystem

            self.quest_system = QuestSystem()
        except ImportError:
            logger.warning("federation_game_quests not available")
        try:
            from federation_game_technology import TechTree

            self.tech_tree = TechTree()
        except ImportError:
            logger.warning("federation_game_technology not available")
        try:
            ...
            logger.warning("federation_game_history_arc not available")
        # Rival Federation Simulator
        try:
            ...
            self.rival_simulator = None
        # USS Chaosbringer Consciousness
        try:
            from uss_chaosbringer_consciousness import FederationConsciousnessEngine

            qc_ref = self.history_arc.consciousness if self.history_arc else None
            ...
            self.chaosbringer = None
        # Technology Engine Adapter
        try:
            from federation_game_technology_integration import TechnologyEngine

            treasury = (
                self.state_manager.federation.treasury if self.state_manager else 1000
            )
            self.tech_tree = TechnologyEngine(treasury=treasury)
        except ImportError:
            self.tech_tree = None
        # Quest Engine Adapter
        try:
            from federation_game_quest_integration import QuestEngine

            self.quest_system = QuestEngine()
            self.quest_system.initialize()
        except ImportError:
            self.quest_system = None
        # NPC System Adapter
        try:
            from federation_game_npc_integration import NPCSystemAdapter
            from federation_game_history_arc import FACTION_IDS

            self.npc_system = NPCSystemAdapter(FACTION_IDS)
            self.npc_system.initialize()
        except ImportError:
            self.npc_system = None

        # Political Engine Adapter
        try:
            from federation_game_history_arc import FACTION_IDS
            from federation_game_political_integration import PoliticalEngine

            self.political_engine = PoliticalEngine(
                FACTION_IDS, self.state_manager.federation
            )
    ...
    def execute_turn(self) -> Dict[str, Any]:
        ...
        for phase in self.turn_cycle.PHASES:
            phase_result = self.turn_cycle.execute_phase(
                phase, {"turn": self.turn_number}
            )
        ...
        # Advance integrated subsystems
        if self.state_manager:
            self.state_manager.advance_turn()
        if self.faction_system:
            self.faction_system.advance_turn()
        if self.history_arc:
            arc_result = self.history_arc.advance_year()
            if arc_result.get("event"):
                self._log_event("history_arc_event", arc_result.get("event", {}))
        if self.rival_simulator:
            self.rival_simulator.advance_year()
        if self.chaosbringer:
            self.chaosbringer.process_year(self.turn_number)
        # Technology adapter: process yearly research
        if self.tech_tree and self.state_manager:
            treasury = self.state_manager.federation.treasury
            completed = self.tech_tree.process_year(self.turn_number, treasury)
        # Quest adapter: process yearly quest updates
        if self.quest_system and self.state_manager:
            try:
                quest_results = self.quest_system.process_year(
                    self.turn_number, self.state_manager
                )
                if quest_results:
                    for quest_result in quest_results:
                        self._log_event(
                            "quest_completed",
                            {
                                "quest_id": quest_result.get("quest_id", "unknown"),
                                "quest_title": quest_result.get(
                                    "quest_title", "Unknown Quest"
                                ),
                            },
                        )
                # Apply quest rewards to federation
                if hasattr(self.quest_system, "apply_rewards"):
                    self.quest_system.apply_rewards(
                        quest_results, self.state_manager.federation
                    )
            except Exception as e:
                logger.warning(f"Quest processing error: {e}")

        # NPC adapter: process yearly advisor bonuses
        if self.npc_system and self.state_manager:
            modifiers = self.npc_system.process_year(
                self.turn_number, self.state_manager
            )
            if modifiers:
                if "morale_delta" in modifiers:
                    self.state_manager.federation.morale = max(
                        0.0,
                        min(
                            1.0,
                            self.state_manager.federation.morale
                            + modifiers["morale_delta"],
                        ),
                    )
                if "identity_delta" in modifiers:
                    self.state_manager.federation.identity_strength = max(
                        0.0,
                        min(
                            1.0,
                            self.state_manager.federation.identity_strength
                            + modifiers["identity_delta"],
                        ),
                    )
        # Political adapter: process yearly law generation
        if self.political_engine and self.state_manager:
            laws = self.political_engine.process_year(
                self.turn_number, self.state_manager.federation
            )
            if laws:
                self.state_manager.political_data = self.political_engine.summary
                for law in laws:
                    self._log_event(
                        "law_passed",
                        {
                            ...
                            "year": self.turn_number,
                            "effects": law.get("effects"),
                        },
                    )
        # Record choice in ledger if there was an event
        if self.current_event:
            self.choice_ledger.append(
                {
                    "turn": self.turn_number,
                    "event_id": getattr(self.current_event, "event_id", "unknown"),
                    "timestamp": datetime.now().isoformat(),
                }
            )
    ...
    def trigger_event(self, event_type: Optional[EventType] = None) -> EventCard:
        ...
        self._log_event(
            "event_triggered", {"event_id": event.event_id, "title": event.title}
        )
        return event
    ...
    def resolve_event(self, choice: str) -> Tuple[str, Dict[str, float]]:
        ...
        # Update last choice ledger entry with actual choice
        if self.choice_ledger and self.choice_ledger[-1].get("choice") == "pending":
            self.choice_ledger[-1]["choice"] = choice
            self.choice_ledger[-1]["outcome"] = outcome
        self._log_event(
            "event_resolved",
            {
        ...
        if self.state_manager:
            self.state_manager.record_action(
                action_type="event",
                description=f"Event: {getattr(self.current_event, 'title', 'unknown')}",
                outcome=outcome,
                morale_delta=impact.get("morale", 0.0),
                stability_delta=impact.get("anxiety", 0.0),
            )
    ...
    def trigger_chaos(self) -> Tuple[str, str, str]:
        ...
        narrative = ChaosMode.generate_chaos_narrative(subsystem)
        ...
        self._log_event(
            "chaos_triggered", {"subsystem": subsystem, "scenario": scenario}
        )
    ...
    def cmd_turn(self):
        ...
        print(
            self.narrative.generate_turn_narrative(
                self.turn_number - 1, self.consciousness
            )
        )
        print("Turn complete.\n")
    ...
    def cmd_rivals(self):
        ...
        if self.rival_simulator:
            summary = (
                self.rival_simulator.get_summary()
                if hasattr(self.rival_simulator, "get_summary")
                else str(self.rival_simulator)
            )
            print(f"\n{summary}\n")
        else:
            print(self.get_rival_summary())
    ...
    def cmd_prophecy(self):
        ...
        print(
            f"Prophecy recorded. Total prophecies: {len(self.consciousness.prophecies)}\n"
        )
    ...
    def cmd_save(self):
        ...
        # Persist adapter summaries
        if self.tech_tree:
            try:
                game_data["tech_summary"] = self.tech_tree.summary
            except Exception:
                logger.debug("Tech tree summary unavailable; omitting from save data")
        if self.quest_system:
            try:
                game_data["quest_summary"] = self.quest_system.summary
            except Exception:
                logger.debug("Quest system summary unavailable; omitting from save data")
        if self.npc_system:
            try:
                game_data["npc_summary"] = self.npc_system.summary
            except Exception:
                logger.debug("NPC system summary unavailable; omitting from save data")
        if self.political_engine:
            try:
                game_data["political_summary"] = self.political_engine.summary
            except Exception:
                logger.debug(
                    "Political engine summary unavailable; omitting from save data"
                )
        # Persist history arc state
        if self.history_arc:
            try:
                game_data["history_arc_state"] = self.history_arc.export_full_state()
            except Exception:
                logger.debug(
                    "History arc state export unavailable; omitting from save data"
                )
    ...
    def cmd_factions(self):
        """Display faction information"""
        print(f"\n{'=' * 70}")
        print("FACTION SYSTEM")
        print(f"{'=' * 70}")
        if not self.faction_system:
            print("Faction system not available.")
            print(f"{'=' * 70}\n")
            return
        for faction_id, faction in self.faction_system.factions.items():
            print(f"  {faction.name} [{faction.ideology.value}]")
            print(f"  ID: {faction_id} | HQ: {faction.headquarters_location}")
            print(f"  Philosophy: {faction.faction_philosophy[:60]}...")
            print(f"  Members: {faction.member_count} | Level: {faction.faction_level}")
            if faction.ally_factions:
                print(f"  Allies: {', '.join(faction.ally_factions)}")
            if faction.enemy_factions:
                print(f"  Enemies: {', '.join(faction.enemy_factions)}")
            print()
        print(f"{'=' * 70}\n")
    ...
    def cmd_npcs(self):
        ...
        if not self.npc_system:
            print("NPC system not available.")
            print(f"{'=' * 70}\n")
            return
        chars = getattr(self.npc_system, "characters", {})
        if not chars:
            print("  No NPCs generated yet.")
        for npc_id, npc in chars.items():
            name = getattr(npc, "name", str(npc))
    ...
    def cmd_quests(self):
        ...
        if not self.quest_system:
            ...
            print(f"{'=' * 70}\n")
            return
        quests = getattr(self.quest_system, "quests", {})
        active = getattr(self.quest_system, "active_quests", [])
        if not quests and not active:
            print("  No quests available yet.")
        for qid, quest in quests.items() if isinstance(quests, dict) else []:
            name = getattr(quest, "name", str(quest))
            diff = getattr(quest, "difficulty", "?")
            print(f"  {name} [{diff}]")
        print(f"{'=' * 70}\n")
    ...
    def cmd_tech(self):
        ...
        if not self.tech_tree:
            print("Technology tree not available.")
            print(f"{'=' * 70}\n")
            return
        techs = getattr(self.tech_tree, "technologies", {})
        researched = getattr(self.tech_tree, "researched_techs", [])
        if not techs:
            print("  No technologies loaded.")
        else:
            total = len(techs) if isinstance(techs, dict) else 0
            done = len(researched) if isinstance(researched, list) else 0
            print(f"  Technologies: {done}/{total} researched")
        print(f"{'=' * 70}\n")

    def cmd_history(self):
        """Display history arc status"""
        print(f"\n{'=' * 70}")
        print("HISTORY ARC")
        print(f"{'=' * 70}")
        if not self.history_arc:
            print("History arc system not available.")
            print(f"{'=' * 70}\n")
            return
        status = self.history_arc.get_status()
        tl = status.get("timeline", {})
        cs = status.get("consciousness", {})
        gs = status.get("game_state", {})
        print(f"  Year: {tl.get('current_year', '?')} | Era: {tl.get('era', '?')}")
        print(
            f"  Events: {tl.get('event_count', 0)} | Factions: {tl.get('faction_count', 0)}"
        )
        print(
            f"  Observers: {cs.get('observer_count', 0)} | Interpretations: {cs.get('total_interpretations', 0)}"
        )
        print(f"  Coherence: {cs.get('coherence', 0.0):.3f}")
        print(f"  Lost possibilities: {cs.get('lost_possibilities_count', 0)}")
        print(f"  Decisions recorded: {status.get('choice_ledger_count', 0)}")
        print(f"{'=' * 70}\n")

    def cmd_choose(self):
        """Record a manual choice/decision"""
        choice_text = input("Enter decision: ").strip()
        if not choice_text:
        ...
        self._log_event("choice_recorded", {"choice": choice_text})
        print(f"Decision recorded. Total decisions: {len(self.choice_ledger)}\n")

    def cmd_reset(self):
        """Reset game state"""
        confirm = input("Reset all game state? (yes/no): ").strip().lower()
        if confirm != "yes":
    ...
    def cmd_bridge(self):
        """Display USS Chaosbringer bridge status"""
        if self.chaosbringer:
            status = self.chaosbringer.get_bridge_status()
            print(f"\n=== USS Chaosbringer Bridge Status ===")
            print(f"Ship State: {status.get('ship_state', 'unknown')}")
            print(f"Alert Level: {status.get('alert_level', 'unknown')}")
            wc = status.get("warp_core", {})
            print(
                f"Warp Core: {wc.get('status', 'unknown')} (coherence: {wc.get('coherence', 0):.2f})"
            )
            shields = status.get("shields", {})
            if shields:
                print(f"Shields: {shields.get('strength', 0):.2f}")
            crew = status.get("crew_stations", {})
            if crew:
                print(f"Crew Stations: {len(crew)} active")
        else:
            print("Chaosbringer consciousness not available.")

    def cmd_politics(self):
        """Display political system status"""
        if not self.political_engine:
            print("Political system not available.\n")
            return
        summary = self.political_engine.summary
        print("=== POLITICAL SYSTEM ===")
        print(f"  Enabled: {summary.get('enabled', False)}")
        print(f"  Laws Passed: {summary.get('laws_passed_count', 0)}")
        print(f"  Cabinet Size: {summary.get('cabinet_size', 0)}")
        recent = summary.get("recent_laws", [])
        if recent:
            print("  Recent Laws:")
            for law in recent[-5:]:
                print(
                    f"   - {law.get('law_name', 'Unknown')} (ID: {law.get('law_id', '?')})"
                )
        print()
        print(f"{'=' * 70}\n")
