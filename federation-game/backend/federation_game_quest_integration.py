...
class QuestEngine:
    ...
    def __init__(self):
        ...
        self.quest_completion_chance: float = 0.3   # 30% chance per year to attempt a quest
        self.reward_scale: float = 0.2              # Scale down resource rewards to prevent inflation
    ...
    def process_year(self, year: int, game_state) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Accept quest
            success, msg = self.quest_system.accept_quest(self.player_id, quest.quest_id, current_turn=year)
            if not success:
                return []
                ...
                if not obj.is_complete():
                    ...
                    if needed > 0:
                        prog_success, prog_msg = self.quest_system.progress_objective(
                            self.player_id, quest.quest_id, obj.objective_id, amount=needed
                        )
                        if not prog_success:
                            # Direct set
            ...
            if comp_success and reward:
                ...
                if 'resources' in reward_dict:
                    reward_dict['resources'] = int(reward_dict['resources'] * self.reward_scale)
                rewards_list.append({
            ...
            # Silently ignore errors to keep simulation running
            pass
    ...
    def apply_rewards(self, rewards_list, federation):
        ...
        for reward_info in rewards_list:
            ...
            if 'reputation' in reward:
                federation.morale = max(0.0, min(1.0, federation.morale + reward['reputation'] * 0.5))
            if 'morale_boost' in reward:
                federation.morale = max(0.0, min(1.0, federation.morale + reward['morale_boost']))
            if 'stability_boost' in reward:
                federation.stability = max(0.0, min(1.0, federation.stability + reward['stability_boost']))
            if 'tech_points' in reward:
                bump = reward['tech_points'] / 1000.0
                federation.technological_level = max(0.0, min(1.0, federation.technological_level + bump))
