...
class GameTurn:
    ...
    def _setup_subsystems(self):
        ...
        self.rival_simulator = None # Connect via attach_rival_simulator() or auto_attach_rival_simulator()
    ...
    def auto_attach_rival_simulator(self) -> bool:
        """Auto-attach the rival simulator if the module is available."""
        try:
            from federation_game_rival_simulator import RivalFederationSimulator
            sim = RivalFederationSimulator()
            sim.initialize_rivals()
            self.attach_rival_simulator(sim)
            return True
        except ImportError:
            return False
