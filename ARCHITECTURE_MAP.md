...
**VPS:** srv1345984.hstgr.cloud | **Domain:** federation-game.deliberatefederation.cloud
...
| Worker | Python (7-phase tick pipeline every 60s) | federation-game-worker-1 | — |
| Database | PostgreSQL 15-alpine | federation-game-postgres-1 | 5432 |
| Cache | Redis 7-alpine | federation-game-redis-1 | 6379 |

**Volume mount:** Backend source is bind-mounted from `/docker/federation-game/backend/` → mounted read-only at `/app/` inside the backend container. Frontend HTML is bind-mounted from `/docker/federation-game/public_html/` → served by nginx.

**Database:** `federation_game` DB exists on postgres container with `game_snapshots` table — periodic state persistence. NPC state lives in Redis, not in Python memory.

---

## 1a. Observability Stack (6 additional containers)

| Service | Container | Port | Purpose |
|---------|-----------|------|---------|
| Prometheus | federation-game-prometheus-1 | 9090 | Metrics collection from all exporters |
| Grafana | federation-game-grafana-1 | 3001 | Dashboards for NPC state, tick performance, Redis/Postgres health |
| cAdvisor | federation-game-cadvisor-1 | 8080 | Container resource monitoring (CPU, memory, network) |
| Redis Exporter | federation-game-redis-exporter-1 | 9121 | Redis metrics (keys, memory, connected clients) |
| Postgres Exporter | federation-game-postgres-exporter-1 | 9187 | PostgreSQL metrics (connections, table sizes, queries) |
| Node Exporter | federation-game-node-exporter-1 | 9100 | Host-level metrics (CPU, disk, network) |

---

## 1b. Traefik Routing Architecture

Traefik routes requests by path with priority-based rules:

**Priority 200 (frontend-first):** Specific `.html` files route directly to the nginx frontend container:
- `/starmap.html`, `/simulation.html`, `/bridge.html`, `/earth.html`
- `/constellation.html`, `/worldguide.html`, `/adult.html`, `/index.html`, `/spectator.html`

**Lower priority (backend-catch):** All `/api/*` paths route to the FastAPI backend container. The nginx config strips the `/api/` prefix before proxying to `http://backend:8000/`.

**Critical:** Any new HTML page added to `public_html/` MUST also be added to the priority-200 Traefik rule in `docker-compose.yml` (both HTTPS and HTTP rules), otherwise the backend catches the path and returns 404 JSON.
...
All game state lives in Redis (NPC thoughts, moods, faction data, tick results). The `GameState` class initializes from Redis on startup. PostgreSQL stores periodic snapshots via the `game_snapshots` table for long-term persistence.
...
- `npc_system: NPCSystem` — 39 characters (13 faction-affiliated), 8 creatures
...
**Dialogue:** LLM-powered via multi-provider router. NPC thoughts and dialogue are generated by `npc_autonomy.py` + `npc_cognition.py` using the LLM router (Ollama 3B local default, Cloudflare Workers AI, Together, NIM, Gemini, Grok, OpenRouter). Pre-written `DialogueNode` trees serve as fallback when LLM is unavailable.
...
### 4.4 Operator Views (TV-optimized for 50-inch QLED)

All views are designed for readability at TV viewing distance — large fonts, high contrast, minimal scrolling.

| Page | File | Purpose |
|------|------|---------|
| Starship Simulator | `index.html` | LCARS kid-friendly interface |
| Adult Control Plane | `adult.html` | Full-depth strategic interface |
| Starmap | `starmap.html` | Interactive star map with faction territories, NPC labels, convex hull polygons, watchlist cards |
| Live Sim | `simulation.html` | Real-time NPC thought feed, faction events, news anchor layer, severity labels |
| Bridge | `bridge.html` | Bridge command interface |
| Earth | `earth.html` | Earth overview |
| Constellation | `constellation.html` | Constellation viewer |
| Spectator | `spectator.html` | Spectator / read-only mode |
| World Guide | `worldguide.html` | Lore, NPC roster, faction descriptions — answers "what is going on?" |

**All 9 pages link to each other via a shared navigation bar.** New pages must be added to the Traefik priority-200 routing rule in `docker-compose.yml` or they will 404 (backend catches the path instead).

---

## 5. Database & State Architecture

**PostgreSQL:** `game_snapshots` table stores periodic state snapshots. SQLAlchemy and psycopg2-binary are in requirements.txt and actively used. Snapshots are taken by the worker tick pipeline and on explicit save requests.

**Redis:** Primary state store. All NPC state (thoughts, moods, goals, memories), tick results, faction dynamics, and world events live in Redis. The worker pipeline reads/writes Redis every 60s tick cycle. Keys are namespaced by NPC ID, faction ID, and world-state category.

---

## 6. LLM / AI Integration

**Multi-provider LLM router (`llm_router.py`):** Routes NPC thought generation, dialogue, and narrative to the best available provider:

| Provider | Model | Role |
|----------|-------|------|
| Ollama (local) | 3B default | Primary local inference — no API cost, no network latency |
| Cloudflare Workers AI | @cf/meta/llama-3-8b-instruct | Remote fallback |
| Together | togethercomputer/LLaMA-2 | Remote fallback |
| NIM | nvidia/nemotron | Remote fallback |
| Gemini | gemini-pro | Remote fallback |
| Grok | grok-beta | Remote fallback |
| OpenRouter | auto | Last-resort router |

**NPC autonomy pipeline (`npc_autonomy.py` + `npc_cognition.py`):**
- Every 60s tick: each of 39 NPCs generates a thought, updates mood, evaluates goals
- Thoughts are LLM-generated using personality context, faction pressure, and recent memories
- Mood is computed from thought sentiment + faction alignment + memory salience
- Fallback to template-based dialogue when all LLM providers are unavailable

**Secrets management:** API keys stored in `.secrets/api-keys.txt` (git-ignored). Loaded into container environment via `.env` file.
...
```
Returns a simple Bearer token. No database-backed auth. Save/load now persists to PostgreSQL via `game_snapshots` table.

---

## 8. Key Architectural Observations

1. **Redis is the primary state store** — NPC data, tick results, faction dynamics all live in Redis. PostgreSQL handles periodic snapshots for long-term persistence.
...
3. **Massive single-file modules** — main.py is ~5796 lines, npc_autonomy.py is ~2422 lines, console.py is ~1923 lines, rival_simulator is ~2133 lines
4. **LLM-powered NPC autonomy** — 39 NPCs generate thoughts, moods, and goals every 60s via multi-provider LLM router. Template fallback when LLM is unavailable.
5. **Worker runs full tick pipeline** — 7-phase cycle every 60s: dream generation, rival actions, diplomacy shifts, prophecy updates, consciousness evolution, random events, status updates
6. **9 frontend views** — Starship (index), Adult Control (adult), Starmap, Live Sim (simulation), Bridge, Earth, Constellation, Spectator, World Guide
7. **Traefik priority-based routing** — Specific `.html` paths route to frontend (priority 200) before backend catches PathPrefix routes. New HTML pages must be added to the Traefik rule.
8. **Full observability stack** — Prometheus, Grafana, cAdvisor, redis-exporter, postgres-exporter, node-exporter
9. **Governance/explainability system** — every choice produces deltas, explainability data, rights impact, rival effects, political effects, and history arc data
10. **WebSocket** — ConnectionManager with basic connect/disconnect/broadcast for real-time updates
...
|------|------|-------|
| main.py | ~220KB | ~5796 |
| npc_autonomy.py | ~92KB | ~2422 |
| worker.py | ~24KB | ~631 |
| map_endpoints.py | ~17KB | ~440 |
| federation_game_npcs.py | 60KB | ~1696 |
...
| quests.py | 41KB | ~580+ |
| starmap.html | ~85KB | ~1400+ |
| simulation.html | ~50KB | ~1000+ |
| index.html | 54KB | — |
...
1. **Docker-served** (`frontend/` + `public_html/`): 9 single-file HTML apps — served via nginx container on port 3000. HTML is bind-mounted from `public_html/` so updates require copy + restart (no rebuild).
...
- PostgreSQL `game_snapshots` table — periodic state snapshots via worker pipeline
- Redis — primary state store for all NPC data, tick results, faction dynamics
- SQLAlchemy/psycopg2 — actively used for snapshot read/write
- `FederationConsole` has a `PersistenceManager` for filesystem JSON (console-only)

### LLM Architecture
- Multi-provider router (`llm_router.py`) — Ollama 3B (local default), Cloudflare, Together, NIM, Gemini, Grok, OpenRouter
- NPC autonomy pipeline (`npc_autonomy.py` + `npc_cognition.py`) generates thoughts, moods, goals per tick
- Template-based dialogue serves as fallback when all LLM providers unavailable
- API keys stored in `.secrets/api-keys.txt` (git-ignored)

### Infrastructure
- Docker-compose mounts backend as read-only (`:ro`) into container — code changes require volume remount or container restart
- Redis is the primary state store — worker.py writes NPC state every 60s tick
- Frontend HTML is bind-mounted from `public_html/` into nginx — copy files + restart frontend container
- **Traefik routing:** Priority-200 rule routes specific `.html` paths to frontend BEFORE backend catches PathPrefix routes. New HTML pages MUST be added to this rule or they 404.
...
1. **Sim status plain-English layer** — Add human-readable interpretation to simulation.html status (from `may24gameplay2(2).txt`)
2. **Real auth** — Database-backed user system with proper token management
3. **Consolidate frontends** — merge best of both codebases (LCARS theme + modular ES architecture + localStorage backup)
4. **Modularize main.py** — ~5796-line monolith should be split into route modules (api_routes.py, simulation_routes.py, etc.)
5. **WebSocket rooms** — Channel-based subscriptions instead of global broadcast
